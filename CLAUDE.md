...
- **Cache-Semantik (2026-07-12)**: Der Generation-Cache bedient nur **Erst-Anfragen** (andere Nutzer, Fehler-Retry). Wiederholt derselbe Nutzer identische Parameter, hat er das Rezept schon → Server schaltet automatisch auf `regenerate` (Variations-Hint) und liefert ein **neues** Rezept. Nie wieder „gleiches Rezept nochmal vorgeschlagen".
...
- **PWA**: Service Worker mit versioniertem Cache `zauberkoch-vN` (aktuell **v10**) — bei jedem App-Shell-Change bumpen und Version hier nachführen. Favoriten offline lesbar.
