...
- **PWA**: Service Worker mit versioniertem Cache `zauberkoch-vN` (aktuell **v11**) — bei jedem App-Shell-Change bumpen und Version hier nachführen. Favoriten offline lesbar.
- **Karten-Motive**: Flache Vektor-Illustrationen pro Gericht-/Glas-Typ in `RecipeMotif.tsx` (5 Motive: highball/tumbler/coupe/pasta/bowl; Matcher `motifForRecipe`, Cocktails matchen über das `glas`-Feld der Listen-API). Neue Motive IMMER über den Skill `.claude/skills/recipe-motifs/` erzeugen (Stil-Spezifikation + Generator-Prompt) — keine KI-Rasterbilder.
