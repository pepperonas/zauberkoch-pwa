...
- **PWA**: Service Worker mit versioniertem Cache `zauberkoch-vN` (aktuell **v24**) — bei jedem App-Shell-Change bumpen und Version hier nachführen. Favoriten offline lesbar.
- **Karten-Motive**: Flache Vektor-Illustrationen pro Gericht-/Glas-Typ in `RecipeMotif.tsx` (22 Kategorien × Varianten = 41 Grafiken, deterministisch per Titel-Hash `variantFor`: 8 Glas-Typen highball/tumbler/coupe/tiki/martini/wine/flute/mule + 14 Gericht-Typen pasta/bowl/suppe/pfanne/pizza/salat/burger/fisch/steak/dessert/taco/auflauf/pancakes/sandwich; Matcher `motifForRecipe`, Cocktails matchen über das `glas`-Feld der Listen-API). Neue Motive IMMER über den Skill `.claude/skills/recipe-motifs/` erzeugen (Stil-Spezifikation + Generator-Prompt) — keine KI-Rasterbilder.
