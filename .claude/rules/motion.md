...
`prefers-reduced-motion` / kein `startViewTransition` → react-router navigiert normal ohne VT. **Title-Bar-Springen auf Mobile (2026-07-15 gefixt):** `env(safe-area-inset-top)` im Header-Padding + `viewport-fit=cover` → im Browser-**Tab** togglet Androids Edge-to-Edge-Adressleisten-Auto-Hide den Inset (0 ↔ Statusleiste), was die sticky Header-Höhe mitten in der Navigation springen ließ. Fix: Safe-Area-Padding nur `@media (display-mode: standalone)` (dort stabil, kein Auto-Hide), im Tab festes `var(--space-3)`. (Reines Adressleisten-Ein/Ausblenden bleibt Browser-Chrome — nur via PWA-Install ganz weg.)
